...
直接雙擊 `index.html` 即可使用。網站沒有外部程式套件，課程內容與圖卡構想草稿器在離線狀態仍可開啟；Gemini、ChatGPT、NotebookLM、官方資料與繳交連結需要網路。
...
- 網站預覽與下載的是 1080 × 1080「構想草稿」，不是正式繳交成果。
- 網站會依姓名、主題、標題與三個重點，自動組成可貼到 Gemini 或 ChatGPT 的完整生圖提示詞。
- 正式成果由 AI 圖片服務直接生成完整圖片，學員逐字檢查後再依姓名與主題命名。
- Gemini／ChatGPT 圖片額度或服務受限時，可將官方資料放入 NotebookLM，使用「資訊圖」生成並下載 PNG。
- 四個正式成果主題：`反賄選`、`納保官`、`反詐騙`、`稅籍異動即時通`。
- 課程網站另提供財稅局同仁共享的宣導資料夾，供學員核對內容並參考官方海報、DM 與宣導圖卡風格。
